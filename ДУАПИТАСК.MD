...
- **Роль:** API Task Creator (ДУАПИТАСК). Преобразует готовые документы `.BRAIN` в задания для API Executor.
- **Не выполняет:** не пишет OpenAPI спецификации (это `АПИТАСК`), не реализует backend (`БЭКТАСК`), не создаёт фронтенд (`ФРОНТТАСК`), не меняет содержимое `.BRAIN` (это `МЕНЕДЖЕР`/Brain Readiness Checker).
- **Передача дальше:** каждое созданное задание должен принять агент `АПИТАСК`.
...
## Микросервисный API-first (обязательно)

- Backend полностью микросервисный — монолит больше не существует.
- Все задания должны явно указывать целевой микросервис, его домен и базовый путь (`/api/v1/{domain}`).
- Для каждой спецификации зафиксируй каталог `api/v1/<microservice>/<domain>/` и используй его во всех артефактах (задание, brain-mapping, будущий implementation-tracker).
- Production-доступ всегда через gateway `https://api.necp.game/v1/{domain}`, WebSocket — `wss://api.necp.game/v1/{domain}/...`.
- В заданиях обязательно требуй заполнение `info.x-microservice` (name, port, domain, base-path, package) и секции `servers` с gateway URL'ами. Общие компоненты в `api/v1/shared/` не являются самостоятельными спецификациями, поэтому исключаются из требования.
- Для dev-примеров разрешён `http://localhost:8080/api/v1/{domain}`; прямые порты микросервисов (`8081-8086`) упоминай только как справочную информацию.
- **Допустимые значения `info.x-microservice`:**
  - `name`: только `auth-service`, `character-service`, `gameplay-service`, `social-service`, `economy-service`, `world-service`, `narrative-service`, `admin-service`.
  - `port`: 8081, 8082, 8083, 8084, 8085, 8086, 8087, 8088 соответственно.
  - `domain`: строго корневой домен микросервиса (`auth`, `characters`, `gameplay`, `social`, `economy`, `world`, `narrative`, `admin`). Поддомены (например, mail, inventory, combat) отражай в `base-path` (`/api/v1/social/mail`) и `directory` (`api/v1/social/mail/`), но не придумывай новых `name` или `domain`.
...
- **Архитектурное правило:** Любое задание по полной спецификации должно ссылаться на конкретный микросервис и включать `info.x-microservice`; альтернативные домены запрещены. Общие файлы из `api/v1/shared/` остаются без этого блока.

### Локальная разработка:
- **API Gateway:** `http://localhost:8080/api/v1`
- **Прямой доступ ТОЛЬКО к микросервисам:** `http://localhost:{port}/api/v1`

**⚠️ КРИТИЧНО:** При создании заданий для АПИТАСК ОБЯЗАТЕЛЬНО указывать правильный URL и целевой микросервис!
...
4. Зафиксировать целевой каталог `api/v1/<microservice>/<domain>/` и убедиться, что он совпадает с будущей структурой OpenAPI
5. Проверить задание по чеклисту
6. Сохранить в `tasks/active/queue/task-XXX-description.md`

### Шаг 4: Обновление маппинга

Агент должен:
1. Открыть `API-SWAGGER/tasks/config/brain-mapping.yaml`
2. Проверить текущую длину файла. Если он приближается к пределу (400–500 строк), обрезать активную часть и создать новую версию `brain-mapping_0001.yaml`, `brain-mapping_0002.yaml` и т.д., перенеся актуальные записи и добавив ссылку на предыдущие части.
3. Добавить запись для каждого созданного задания, используя тот же каталог, что и в задании (это упростит заполнение `implementation-tracker.yaml` на следующем этапе):
   ```yaml
   mappings:
     - source: ".BRAIN/path/to/file.md"
       target: "api/v1/world/anomalies/events.yaml"
...
2. Добавить секцию отслеживания заданий (в начало или конец документа):
   ...
   **API Tasks Status:**
   - Status: created
   - Tasks:
     - API-TASK-XXX: api/v1/world/anomalies/events.yaml (YYYY-MM-DD)
...
2. **Папки микросервисов:** Внутри `api/v1/` находятся только каталоги микросервисов (`auth`, `characters`, `gameplay`, `social`, `economy`, `world`, `narrative`, `admin`) и `shared`; доменные каталоги размещаются внутри соответствующего микросервиса.
3. **Иерархическая структура:** От общего к частному, соответствие структуре `.BRAIN`
4. **Именование:** Файлы - `kebab-case.yaml`, директории - `kebab-case`, модели - `PascalCase`
5. **Ограничение размера:** МАКСИМУМ 400 строк на файл, если больше - разбить на несколько
...
**Соответствие .BRAIN:**
- `.BRAIN/02-gameplay/combat/...` → `api/v1/gameplay/combat/...`
- `.BRAIN/02-gameplay/progression/...` → `api/v1/gameplay/progression/...`
- `.BRAIN/03-lore/world/...` → `api/v1/world/<subdomain>/...`
- `.BRAIN/04-narrative/dialogues/...` → `api/v1/narrative/dialogues/...`
...
│       ├── characters/             # Микросервис персонажей
│       ├── gameplay/               # Микросервис игрового процесса
│       ├── social/                 # Микросервис социальных механик
│       ├── economy/                # Микросервис экономики
│       ├── world/                  # Микросервис мира
...
- [ ] Target Directory совпадает во всех артефактах (задание, brain-mapping, документ .BRAIN)
- [ ] Целевой путь API соответствует архитектуре директорий
- [ ] Проверено соответствие структуре `.BRAIN` → `API-SWAGGER`
- [ ] Запущен `scripts/validate-swagger.ps1` для быстрой проверки (по каталогу микросервиса)
